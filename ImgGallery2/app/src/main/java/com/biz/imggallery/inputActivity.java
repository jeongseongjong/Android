...
public class inputActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_save.setOnClickListener((view) -> {
            ...
            // 나를 호출한 Acitivity의 context 가져오서ㅏㅓ
            // 값을 return 하기 위한 준비
            Intent myIntent = getIntent();

            // model에 "변수", 값 형식으로 Attribute를 설정한 것 같은 원리로
            // 되돌리는 변수(객체, 클래스)등을 설정
            myIntent.putExtra(InputFiledName.TXT_NAME,strName);
            myIntent.putExtra(InputFiledName.TXT_TEL,strTel);
            myIntent.putExtra(InputFiledName.TXT_ADDR, strAddr);
